...
const releaseToken = process.env.RELEASE_MANIFEST_TOKEN;
...
const releaseHeaders = { Accept: 'application/vnd.github+json', 'User-Agent': 'tax-evidence-pack-site-build', ...(releaseToken ? { Authorization: `Bearer ${releaseToken}` } : {}) };
async function fetchJson(url) { const response = await fetch(url, { headers: releaseHeaders, signal: AbortSignal.timeout(12_000) }); if (!response.ok) throw new Error(`release API returned ${response.status}`); return response.json(); }
async function createManifest() {
  try {
    const release = await fetchJson(apiUrl);
    const checksumAsset = release.assets?.find((asset) => asset.name === 'SHA256SUMS');
    let sums = new Map();
    if (checksumAsset?.browser_download_url) try { const response = await fetch(checksumAsset.browser_download_url, { headers: releaseHeaders, signal: AbortSignal.timeout(12_000) }); if (response.ok) sums = parseSums(await response.text()); } catch { /* API digests below are a safe fallback. */ }
